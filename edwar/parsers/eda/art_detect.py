...
def detect_arts(eda_data, classifier_list):
    ...
    features1 = eda_data.groupby(pd.Grouper(freq='5s', origin=eda_data.index[0])).apply(__create_feature_df)
    features = pd.DataFrame(features1.values.tolist(), columns=all_feature_names, index=features1.index)

    # Al calcular las features, si la ultima ventana de 5s contiene muy pocos datos la función __create_feature_df
