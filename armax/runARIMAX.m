function runARIMAX(varargin)
    %dbstop at 7
%     dbstop at 29
%dbstop at 442
    
    %% CONSTANTS
    MAX_NUMBER_OF_INPUTS = 10; % HR, EDA, TEMP, SPO2, 5*EEG, PTT
    ...
    switch mode
        case 'create' % Create a new model from data
            reportFile = strcat('log_Models', upper('armax'),'.txt');
            ...
            modelStruct = struct('stateSystem', model,...
                ...
                'orders',[],...
                'normalization', norm);
            saveARMAXModel(modelStruct, orders, dataRead, fileName);
...
%             % Save and print the results.
%             % Compare model with the original output
%             saveARMAXModel(model, predictedData, fileName);
    end

    
    %% CREATE ARMAX MODEL
    function [stateSystem, data, orders] =...
            createARMAXModel(ordersModel, algorithmOptions, normalization, domainInfo, predictorsNames, yName)
        %% Variables
        % Options for model
        opt = armaxOptions; % Default values
...
%                 otherwise
%                     error('ErrorTests: convertTest', ...
%                         'The option: %s do not exist for ARMAX', name);
%             end
%         end
%         
...
%         else
%             if length(ordersModel) == 4 % ARMAX or ARIMAX if 'IntegrateNoise' option is true.
%                 na = ordersModel{1};
%                 nb = ordersModel{2};
%                 nc = ordersModel{3};
%                 nk = ordersModel{4};
%                 armaxOrders = [na nb nc nk];
...
%         %%
 %%       
    ...
    %% SAVE
    function saveARMAXModel(model, orders, data, name)
...
%          model_mig1= 'model_na14nb14nc5nk5_of_TEMP_EDA_HR_Plux_SPO2_output'
%          model_mig2= 'model_na22nb22nc20nk20_of_TEMP_EDA_HR_Plux_SPO2_output';
%          model_mig3= 'model_na19nb19nc10nk10_of_TEMP_EDA_HR_Plux_SPO2_output';
%          model_mig4= 'model_na18nb18nc20nk20_of_TEMP_EDA_HR_Plux_SPO2_output';
%          for m=1:4
%             % Load model
%             modelMig = eval(strcat('mig',num2str(m)));
%             mod = eval(strcat('model_mig',num2str(m)));
            
%             modelo = strcat(rootDirModels, modelMig, '/algorithms/arimax/na-eq-nb_nc-eq-nk/', mod);
...
%          model_mig1= 'model_na28nb28nc10nk10_of_TEMP_HR_Plux_SPO2_output'
% 	 model_mig2= 'model_na22nb22nc25nk25_of_TEMP_HR_Plux_SPO2_output';
%          model_mig3= 'model_na7nb7nc15nk15_of_TEMP_HR_Plux_SPO2_output';
%          model_mig4= 'model_na13nb13nc20nk20_of_TEMP_HR_Plux_SPO2_output';
%         for m=1:4
%             % Load model
%             modelMig = eval(strcat('mig',num2str(m)));
%             mod = eval(strcat('model_mig',num2str(m)));
...
%         end
 ...
 predictorsNames = {'TEMP', 'EDA', 'HR_Plux'};
         model_mig1= 'model_na11nb11nc30nk30_of_TEMP_EDA_HR_Plux_output';
         model_mig2= 'model_na26nb26nc15nk15_of_TEMP_EDA_HR_Plux_output';
         model_mig3= 'model_na17nb17nc10nk10_of_TEMP_EDA_HR_Plux_output';
         model_mig4= 'model_na19nb19nc15nk15_of_TEMP_EDA_HR_Plux_output';
         % Migrañas 1-4
         for m=1:4
            % Load model
            modelMig = eval(strcat('mig',num2str(m)));
