...
function usage {
    ...
    echo -ne "-m, --mode\t (Required) create or validate. To create or validate an ARX model. Case validate, as parameter, introduce the model name without extension\n"
...
echo -ne "All the possible options for MATLAB's ARX algorithm. Do it in pairs name-value (no key sensitive); e.g. -InitialState estimate -N4Weight CVA\n"
    echo -ne "Example of usage:\n"
    echo -ne "./runARX.sh -m validate model_2na3nb5nc0nk_TEMP_EDA -i ../raw_data/patients/patient_2/data/data_201406021630-201406030230 -v TEMP EDA -o output\n"
}
...
function check_domainData {
    ...
    cat /tmp/optionsARX.csv /tmp/auxNewLineFile.csv > /tmp/optionsARXTemp.csv

    mv /tmp/optionsARXTemp.csv /tmp/optionsARX.csv  
}


## Create a temporal CSV file with options and its values
function check_orderARX {
    if [ "$nx" == "" ]; then 
	nx=`echo "best"`
	echo -en '\E[38;5;202m'"Warning:\n\033[0m" # Orange text
	echo -ne "ARX order set as: $nx\n"
    else
	echo -ne "ARX order: $nx\n"
    fi;

    newLine="orderARX,$nx,"
    echo "$newLine" > /tmp/auxNewLineFile.csv
    cat /tmp/optionsARX.csv /tmp/auxNewLineFile.csv > /tmp/optionsARXTemp.csv
    cp /tmp/optionsARXTemp.csv /tmp/optionsARX.csv
    check_domainData;
}


function create_options_file {
    rm /tmp/auxNewLineFile.csv /tmp/optionsARX.csv 
    touch /tmp/optionsARX.csv  /tmp/optionsARXTemp.csv
    if (( "num_rows" > "0" )); then
    ...
    # Empty file
	echo "$newLine" > /tmp/optionsARX.csv
    fi
...
case $programMode in
    create )
	functionToCall=runAlgorithms"('$algorithName','$programMode','$inputDataPath/',{$variables},{'$output'})"
	;;
    validate )	
	functionToCall=runAlgorithms"('$algorithName','$programMode','$modelName',$predictionTime,'$inputDataPath/',{$variables},{'$output'})"
	;;
    *)	
esac;
# Metric:
